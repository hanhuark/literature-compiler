...
REQUIRED_COLUMNS = {
    "paper_id", "architecture", "point_id", "thermal_resistance_K_W", "pressure_drop_Pa", "volumetric_flow_m3_s", "pumping_power_W", "heat_flux_W_m2", "mass_flux_kg_m2_s", "inlet_pressure_Pa", "inlet_subcooling_K", "stability_state", "thermal_resistance_definition", "pressure_boundary", "source_type", "extraction_method", "include_in_plot",
}
...
def compile_points(points: pd.DataFrame) -> pd.DataFrame:
    ...
    if plotted.empty:
        return compiled
    needed = ["thermal_resistance_K_W", "pumping_power_W", "heat_flux_W_m2", "mass_flux_kg_m2_s", "stability_state", "thermal_resistance_definition", "pressure_boundary"]
    if plotted[needed].isna().any().any() or (plotted["thermal_resistance_K_W"] <= 0).any() or (plotted["pumping_power_W"] <= 0).any():
        raise ValueError("Plot-ready two-phase rows need positive thermal resistance and pumping power plus state, thermal, and pressure metadata")
    return compiled
...
def write_plot(compiled: pd.DataFrame, output_path: Path) -> Path | None:
    ...
    markers = {"stable_boiling": "o", "unstable_boiling": "s", "partial_dryout": "^", "CHF_or_dryout": "X"}
    for (paper_id, architecture, stability), group in plotted.groupby(["paper_id", "architecture", "stability_state"], sort=True):
        group = group.sort_values("pumping_power_W")
        ax.plot(group["pumping_power_W"], group["thermal_resistance_K_W"], marker=markers.get(stability, "o"), linewidth=1.3, markersize=4.5, color=colors[architecture], label=f"{paper_id}: {architecture}, {stability}")
    ax.set_xscale("log")
    ax.set_xlabel("Pumping power, Ppump (W)")
    ax.set_ylabel("Thermal resistance, Rth (K/W)")
    ax.set_title("Two-phase MMC and SMC literature points")
...
def build_case_outputs() -> tuple[Path, Path | None]:
    ...
    plot_path = SUMMARY_DIR / "test5_MMC_heat_sink_two-phase.png"
